...
class PetRecordsComponent extends Component {
  ...
  DeleteRecord = async (RecordName, PetRecordId) => {
    if (window.confirm("You want to delete " + RecordName)) {
      axios.delete(`https://petrecs.herokuapp.com/api/pet-records/` + PetRecordId, {withCredentials: true} )
      .then(response=>{
  ...
  componentDidMount() {
    axios.get(`https://petrecs.herokuapp.com/api/pet-records/pet/` + this.state.PetId, {withCredentials: true} )
      .then(response=>{
        this.setState({records: response.data});
        console.log(response.data);
      })
